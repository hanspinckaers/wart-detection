...
def find_warts(img_path, output, kernel_size=(8, 8), laplacian=(8, 8), morph_iterations=2, blur_size=[7, 60, 60], canny_params=[20, 80, 5], dist_threshold=0.5, dilation_iterations=3, percentile=50):
    ...
    if os.path.isfile("cache/" + img_path.replace("../","/") + "/mask.png"):
        ...
        # luv is a better color space for mean shift (see original paper)
    ...
    print(img_path + " %.2f" % (time.time() - st) + "s" + " - 5. gaussian blur inside of finger")
    ...
    # 'mask' is a matrix where 1 means skin and 0 means other.
    # dilate the mask, so we do not find the edges of the skin in the canny edge detector
